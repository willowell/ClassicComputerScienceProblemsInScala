...
///
package book.chapterEx.oopVisitor.newExpression
...
case class Mod (left: Expr, right: Expr) extends Visitable          // NEW
...
class Interpreter extends Visitor[Double] {
  ...
  override def visit(v: Visitable): Double = v match {
    ...
    case Mod(left: Expr, right: Expr) => visit(left) % visit(right) // NEW
